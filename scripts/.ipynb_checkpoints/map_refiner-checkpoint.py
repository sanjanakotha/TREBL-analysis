...
class MapRefiner:
    """A class to refine sequencing maps made through BarcodeMapper in mapping.py.

    Attributes:
        db_path (str): Path for new or existing DuckDB database.
        cols (list): List of barcode or tile column names to process. At least one must be "AD".
        reads_threshold (int): Minimum reads threshold for filtering map4.
        column_pairs (list): List of (key_cols, target_cols) for enforcing each target maps to one key.
    ...
    def save_loss_table(self, output_csv_path):
        ...
        Example:
            ```python
            refiner.save_map_lengths("map_lengths.csv")
            ```
        """
        map_info = [
            ("map1", "Total reads"),
